(setq key-leap-left-chars '(?a ?o ?e ?u))
(setq key-leap-right-chars '(?s ?n ?t ?h))
...
(defun index-from (keys)
  (let* ((key-list (string-to-list keys))
         ...
         (c2 (nth 1 key-list))
         (c3 (nth 2 key-list))
         (v1 (position c1 key-leap-right-chars))
         (v2 (position c2 key-leap-left-chars))
         (v3 (position c3 key-leap-right-chars)))
    (+ (* 16 v1) (* 4 v2) v3)))
...
(defun keys-to-string (keys)
  (let ((k1 (first keys))
        (k2 (nth 1 keys))
        (k3 (nth 2 keys)))
    (string (nth k1 key-leap-right-chars) (nth k2 key-leap-left-chars) (nth k3 key-leap-right-chars))))
        ...
        (progn
          (unless
              (with-local-quit
                ...
                (setq current-key "")
                (key-leap--update-margin-keys (selected-window))
                (key-leap--append-char key-leap-right-chars)
                (key-leap--update-margin-keys (selected-window))
                (key-leap--append-char key-leap-left-chars)
                (key-leap--update-margin-keys (selected-window))
                (key-leap--append-char key-leap-right-chars)
